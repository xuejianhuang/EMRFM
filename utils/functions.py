...
def save_model(model, model_dir):
    ...
    #save_model = model.model if hasattr(model, 'model') else model
    model_file = os.path.join(model_dir, 'pytorch_model.bin')
    model_config_file = os.path.join(model_dir, 'config.json')
    torch.save(model.state_dict(), model_file)
    with open(model_config_file, "w") as f:
        f.write(json.dumps(model.args.common_hyper_param))
...
def save_results(args, test_results, debug_args = None):
    ...
    if test_results.get('best_eval_score'):
    ...
    label_list = benchmarks[args.dataset]["intent_labels"]
    label=[]
    for y in test_results['y_true']:
        label.append(label_list[y])

    df = pd.DataFrame({'y': test_results['y_true'],
                       'label' : label,
                       'gate_a': test_results['gate_a'],
                       'gate_v': test_results['gate_v']})
    gate_path=os.path.join(args.results_path, args.config_file_name+'_gate.csv')
    df.to_csv(gate_path,index=False)
    #
    confusion_path = os.path.join(args.results_path, args.config_file_name + '_confusion.xlsx')
    confusion_df= pd.DataFrame(confusion_matrix(test_results['y_true'], test_results['y_pred']))
    confusion_df.to_excel(confusion_path,index=False)

    # wr = pd.ExcelWriter(os.path.join(args.results_path, args.config_file_name + '_u_feats.xlsx'))
    # df1 = pd.DataFrame(test_results['private_t'])
    # df2= pd.DataFrame(test_results['private_a'])
    # df3 = pd.DataFrame(test_results['private_v'])
    # df4 = pd.DataFrame(test_results['shared_t'])
    # df5 = pd.DataFrame(test_results['shared_a'])
    # df6 = pd.DataFrame(test_results['shared_v'])
    # df1.to_excel(wr, sheet_name="private_t")
    # df2.to_excel(wr, sheet_name="private_a")
    # df3.to_excel(wr, sheet_name="private_v")
    # df4.to_excel(wr, sheet_name="shared_t")
    # df5.to_excel(wr, sheet_name="shared_a")
    # df6.to_excel(wr, sheet_name="shared_v")
    # wr.save()
