...
class AudioDataset:
    ...
    def __load_feats(self, audio_feats_path, base_attrs):
        ...
        # Mean=0
        # variance=0.01
        # train_feats = [audio_feats[x] + np.random.normal(Mean,variance,(len(audio_feats[x]),768))  for x in base_attrs['train_data_index']]
    ...
    def __padding(self, feat, audio_max_length, padding_mode = 'zero', padding_loc = 'end'):
       ...
       # feat=np.concatenate((feat,[[audio_length]*feat.shape[-1]]))  #HXJ
