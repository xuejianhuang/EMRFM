...
class VideoDataset:
    ...
    def __load_feats(self, video_feats_path, base_attrs):
        ...
        # mean = 0
        # variance = 0.01
        # train_feats = [video_feats[x] +np.random.normal(mean,variance,(len(video_feats[x]),1,256)) for x in base_attrs['train_data_index']]
    ...
    def __padding(self, feat, video_max_length, padding_mode = 'zero', padding_loc = 'end'):
       ...
       # feat = np.concatenate((feat, [[video_length] * feat.shape[-1]]))  # HXJ
